...
- Column 2 now treats the tab rail as the primary navigation surface. The tab panel itself is flex-based so the content area can scroll independently while the rail stays available.
- The Plan tab is no longer duplicated inside the artifact explorer; `plan.md` is shown only once, in the dedicated Plan view.
- Checkpoints and Research are split into separate tabs, and each tab opens file content directly so the explorer shows the full artifact rather than only metadata.
...
- Artifact and database smoke checks confirmed the dedicated artifact tabs rendered without requiring manual filesystem access.
...
- **Column 2** now uses a rail-first inspector with grouped sub-agent handling, dedicated Checkpoints and Research tabs, and a session DB mode switch between table and dependency graph views. The rail remains the primary way to move between the main thread, plan, todos, sub-agent threads, checkpoints, research, and session DB.
